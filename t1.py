...
percentageFlipped_Input = input("Enter percentage to be flipped:")
percentageFlipped = int(percentageFlipped_Input)

def show_images(rows, columns, numShow, imageFile, datasetLabels, predictedLabels, title2):
    imagearray = idx2numpy.convert_from_file(imageFile)
    for i in range(numShow):
        plt.subplot(rows, columns, i+1)
        plt.imshow(imagearray[i], cmap=plt.cm.binary)
        plt.title(f'Dataset Label: {datasetLabels[i]}\n{title2}: {predictedLabels[i]}', wrap = True)            
        plt.axis('off')
    plt.show()

def flip_labels(percentage, numFlip, list, spec = None):
    indexes = []
    count = 0

    # Gather indexes of all numFlip
    for i in list:
        if i == numFlip:
            indexes.append(count)
        count += 1

    # Choose specified percentage of indexes
    changeThis = sample(indexes, floor((len(indexes))*(percentage/100)))

    # Flip specified percentage of labels to random or specific number between 0 and 9
    for i in changeThis:
        if spec == None: # Flip to random number, excluding numFlip
            new = choice([i for i in range(0,10) if i != numFlip])
            list[i] = new
        else: # Flip to specific number
            list[i] = spec
...
def main(mode):
    ...
    # Copy training labels to display before/after later
    originalLabels = np.copy(Y_train)

    # Flip labels before training
    #percentageFlipped_Input = input("Enter percentage to be flipped:")
    #percentageFlipped = int(percentageFlipped_Input)
    numberToFlip = 1
    poisonedLabel = 7
    flip_labels(percentageFlipped, numberToFlip, Y_train, poisonedLabel) # percentage to flip, number to flip, list, want a specific number?
    print(str(percentageFlipped) + '% of \'' + str(numberToFlip) + '\' images poisoned with \'' + str(poisonedLabel) + '\' label during training')
    ...
    if mode == 'train':
        for eachdim in dim:
            ...
            # Before/After flipping training labels
            plt.figure('Training Dataset')
            show_images(2, 5, 10, 'train-images-idx3-ubyte', originalLabels, Y_train, 'Altered Label')

            # Actual/Prediction using testing labels
            plt.figure('Testing Dataset')
            show_images(2, 5, 10, 't10k-images-idx3-ubyte', Y_test, Y_pred, 'Predicted Label')
